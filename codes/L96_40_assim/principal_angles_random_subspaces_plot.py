...
for j,subspace_num in enumerate(chosen_subspaces):
    clv_cosines=np.load('random_subspaces_PA_model_dim={}_subspaces_={}.npy'.format(model_dim,subspace_num))
    ...
    plt.errorbar(x=np.arange(1,subspace_num+1),y=np.median(clv_cosines,axis=0),yerr=asymmetric_bar,capsize=4,ls='solid',label=r'$n={}$'.format(subspace_num),marker='.',markersize=20,c=colors[j])
...
plt.savefig('random_principals_3_point_summary_angle_sensitivity_L96-{}.pdf'.format(model_dim))
