...
for i,mu in enumerate(mus):
    BLV_cosines=np.rad2deg(np.arccos(np.load('clv_cosine_mu={}_model_dim={}_num_cl={}.npy'.format(mu,model_dim,num_BLV))))
    ...
    medians[i]=np.median(BLV_cosines,axis=0)


fig, ax =plt.subplots(figsize=(14,8))  
for i in range(num_BLV):     
    ax.errorbar(x=mus,y=medians[:,i],yerr=bars[:,:,i],capsize=6,mec='black',label=r'$CLV$={}'.format(i+1),lw=3,ls=line_types[i],marker='.',ms=20,c=colors[i])
...
ax2.set_xlabel(r'$RMSE$',fontsize=25)
ax.legend(loc='upper center',ncol=len(mus))
...
os.chdir(plots_path)
plt.savefig('CLV_angle_versus_mu_L63-{}_seed_{}.pdf'.format(model_dim,seed_num),bbox_inches='tight', pad_inches=0)
