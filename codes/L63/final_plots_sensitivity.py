...
for i in range(num_clv):     
    ax.errorbar(x=sigmas,y=medians[:,i],yerr=bars[:,:,i],capsize=6,mec='black',label=r'$CLV$={}'.format(i+1),lw=3,ls=line_types[i],marker='.',ms=20,c=colors[i])

ax.set_xlabel(r'$\sigma \to$',fontsize=30)
ax.set_ylim(0.0,80)
ax.set_xticks(sigmas)
ax.set_ylabel(r'$\theta \to$',fontsize=30)
...
for i in range(num_clv):     
    ax_inset.errorbar(x=sigmas1,y=medians1[:,i],yerr=bars1[:,:,i],capsize=4,ls=line_types[i],lw=3,mec='black',label=r'$CLV$={}'.format(i+1),marker='.',ms=20,c=colors[i])

ax_inset.set_xticks(sigmas1)
#ax_inset.set_ylabel('')
plots_path='/home/shashank/Documents/enkf_for_clv2/plots/L63'
os.chdir(plots_path)
plt.savefig('clv_angle_versus_sigma_L63-{}_seed_{}.pdf'.format(model_dim,seed_num),dpi=300,bbox_inches='tight', pad_inches=0)
